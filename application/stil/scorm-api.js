function SCORM_API_1_2 (id_pokusaja) {
	...
	$.ajax ({
		...
		data:{'sco_id' : encodeURIComponent(SESSION_SCO_ID)}, 
		type:"GET",
		async:false,
		dataType:"json",
		error:function(xhr) {
    ...
    this.LMSFinish = function (value) {
		...
		if (value == "") {
			...
			$.ajax ({
				...
				data:{'sco_id' : encodeURIComponent(), 'masteryscore' : encodeURIComponent()}, 
				type:"GET",
				async:false,
				dataType:"json",
				error:function(xhr) {
    ...
    this.LMSCommit = function (param) {
		...
		$.ajax ({
				url:CI_ROOT + 'index.php/rte/spasi_sesiju/' + id_pokusaja +'/?'+data+'&sco_id='+encodeURIComponent()+"&sco_title="+encodeURIComponent(), 
				data:{}, 
				type:"GET",
				async:false,
				dataType:"json",
